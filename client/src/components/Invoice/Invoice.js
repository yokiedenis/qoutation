...
const Invoice = () => {
  ...
  const [stampUSD, setStampUSD] = useState(0);
  ...
  useEffect(() => {
    ...
    if (invoice) {
      ...
      // Handle stamp duty - determine if it was USD or UGX
      if (invoice.stamp < 50) {
        // It was USD
        setStampUSD(invoice.stamp);
        setExchangeRate(invoice.rates || 0);
      } else {
        // It was UGX
        setStampUSD(0);
        setExchangeRate(0);
  ...
  const handleStamp = (e) => {
    const stampValue = parseFloat(e.target.value) || 0;
    setStampUSD(stampValue);

    // If stamp < 50, treat as USD and ask for exchange rate
    // If stamp >= 50, treat as UGX directly
    if (stampValue < 50) {
      // Still USD, needs conversion
      const convertedStamp = stampValue * exchangeRate;
      setStamp(convertedStamp);
    } else {
      // Treat as UGX directly
      setStamp(stampValue);
      setExchangeRate(0); // Clear exchange rate
    }

    setInvoiceData((prevState) => ({
      ...prevState,
      stamp: stampValue < 50 ? stampValue * exchangeRate : stampValue,
    }));
  };
  ...
  const handleExchangeRate = (e) => {
    ...
    // Recalculate stamp if it's in USD (< 50)
    if (stampUSD < 50) {
      const convertedStamp = stampUSD * rate;
      setStamp(convertedStamp);
      setInvoiceData((prevState) => ({
        ...prevState,
        stamp: convertedStamp,
        rates: rate,
      }));
    }
  };
  ...
  useEffect(() => {
    const total = () => {
      ...
      const subtotalWithFees = subTotal + stickerTotal + levy;

      //VAT is calculated as 18% of (subtotal + sticker fee + training levy)
                  ...
                  <TableRow key={index}>
                    ...
                    <TableCell align="right">
                      ...
                      <InputBase
                        ...
                        name="unitPrice"
                        onChange={(e) => handleChange(index, e)}
                        value={formatNumberWithCommas(itemField.unitPrice)}
                        placeholder="0"
                      />{" "}
                    </TableCell>
                    <TableCell align="right">
            ...
            <Grid container>
              ...
              <Grid item style={{ marginTop: "16px", marginRight: 10 }}>
                <TextField
                  ...
                  placeholder="e.g 14 (USD if <50, UGX if ≥50)"
                  label="Stamp duty"
                />
              </Grid>
              {stampUSD > 0 && stampUSD < 50 && (
                <Grid item style={{ marginTop: "16px", marginRight: 10 }}>
                  <TextField
                    ...
                    label="Exchange Rate (USD to UGX)"
                    value={exchangeRate}
                  />
                </Grid>
              )}
